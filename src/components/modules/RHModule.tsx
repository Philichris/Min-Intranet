...
import { Users, Plus, Download, Trash2, FileText, Search, Settings, Edit2, X, Folder } from 'lucide-react';
...
export const RHModule: React.FC = () => {
  ...
  const serviceObj = services.find(s => s.code === 'RH');
  const serviceTitle = serviceObj ? serviceObj.name : 'Ressources Humaines';
  const serviceDesc = serviceObj ? serviceObj.description : 'Formulaires d\'acomptes, CET, contrats de travail et rubriques paramétrables.';
  ...
  const categories: CategoryTab[] = (serviceObj?.subServices && serviceObj.subServices.length > 0)
    ...
    : [
        { id: 'acompte', name: 'Demandes d’Acompte', label: '1. Demandes d’Acompte', desc: 'Formulaires et avances sur salaire', iconName: 'Users' },
      ];

  const [activeTab, setActiveTab] = useState<string>(categories[0]?.id || 'acompte');
  ...
  const [subFolders, setSubFolders] = useState<Array<{ id: string; name: string; categoryId: string }>>(() => {
    try {
      const saved = localStorage.getItem('min_rh_subfolders');
      if (saved) return JSON.parse(saved);
    } catch (e) {}
    return [
      { id: 'sf-rh-1', name: '2026_Dossiers_Acomptes', categoryId: 'acompte' },
    ];
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem('min_rh_subfolders', JSON.stringify(subFolders));
    } catch (e) {}
  }, [subFolders]);
  ...
  const defaultRHDocs: Record<string, Array<any>> = {
    acompte: [
      { id: 'rh1', title: 'Formulaire type - Demande d’acompte sur salaire', category: 'acompte', date: '2026-01-15', size: '320 Ko', type: 'PDF', ref: 'RH-ACOMPTE', uploader: 'Service RH' },
      { id: 'rh2', title: 'Barème et échéancier des acomptes exceptionnels', category: 'acompte', date: '2026-02-01', size: '450 Ko', type: 'PDF', ref: 'RH-BAREME', uploader: 'Service RH' }
    ],
    cet: [
      { id: 'cet1', title: 'Demande d’approvisionnement Compte Épargne-Temps (CET)', category: 'cet', date: '2026-01-10', size: '510 Ko', type: 'PDF', ref: 'RH-CET-01', uploader: 'Service RH' }
    ],
    contrats_rh: [
      { id: 'con1', title: 'Modèle type - Contrat de travail CDI (Cadre / Etam)', category: 'contrats_rh', date: '2026-01-05', size: '890 Ko', type: 'DOCX', ref: 'CDI-MODELE', uploader: 'Juridique Social' }
    ],
    procedures_rh: [
      { id: 'p1', title: 'Guide d’accueil et parcours d’intégration des nouveaux collaborateurs', category: 'procedures_rh', date: '2026-03-01', size: '3.4 Mo', type: 'PDF', ref: 'GUIDE-ACCUEIL', uploader: 'RH' }
    ]
  };
  ...
  const globalMappedDocs = serviceDocs.map(d => ({
    ...
    uploader: d.authorName || 'Service RH',
    fileUrl: d.fileUrl,
    fileName: d.fileName
  }));

  const tabSubFolders = subFolders.filter(sf => sf.categoryId === effectiveCat);
  const tabDefaults = defaultRHDocs[effectiveCat] || [];
  ...
  const handleDeleteCategory = (catId: string, e: React.MouseEvent) => {
    ...
    openModal('confirm', {
      ...
      message: 'Voulez-vous vraiment supprimer cette rubrique et ses documents ?',
      onConfirm: () => {
        if (serviceObj) {
  ...
  return (
    <div className="space-y-8 p-8 max-w-7xl mx-auto bg-slate-50/50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-6 rounded-3xl border border-indigo-200 shadow-sm relative overflow-hidden">
        ...
        <div className="pl-3">
          <div className="inline-flex items-center gap-2 rounded-full bg-indigo-50 px-3 py-1 text-xs font-extrabold text-indigo-700 border border-indigo-200 mb-1">
            <Users className="h-3.5 w-3.5 text-indigo-600" />
            {serviceObj ? serviceObj.name : 'Direction des Ressources Humaines'}
          </div>
              ...
              {isAdmin && (
                <div className="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className={`p-1 rounded-lg ${isActive ? 'bg-indigo-500 text-white' : 'bg-slate-100 text-slate-600'}`}
                  >
                    <Edit2 className="h-3 w-3" />
                  </button>
                  <button
                    onClick={(e) => handleDeleteCategory(cat.id, e)}
                    className={`p-1 rounded-lg ${isActive ? 'bg-indigo-500 text-white hover:bg-rose-500' : 'bg-slate-100 text-rose-600'}`}
                  >
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl relative">
            ...
            <h3 className="text-lg font-bold text-slate-900 mb-2">Ajouter un Document RH</h3>
            <form onSubmit={handleAddDoc} className="space-y-4">
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-slate-200">
                ...
                <button type="submit" className="rounded-xl bg-indigo-600 text-white px-5 py-2 text-xs font-bold hover:bg-indigo-500 shadow-md">Enregistrer</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showEditModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl relative">
            ...
            <h3 className="text-lg font-bold text-slate-900 mb-2">Modifier le Document RH</h3>
            <form onSubmit={handleUpdateDoc} className="space-y-4">
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-slate-200">
                ...
                <button type="submit" className="rounded-xl bg-indigo-600 text-white px-5 py-2 text-xs font-bold hover:bg-indigo-500 shadow-md">Mettre à jour</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl relative">
            ...
            <form onSubmit={handleCreateSubFolder} className="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  placeholder="Ex: 2026_Campagne_Evaluations"
